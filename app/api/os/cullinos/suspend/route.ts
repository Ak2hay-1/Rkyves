...
export async function POST(req: NextRequest) {
  ...
  await db.update(schema.cullinosTenants).set({ status: "suspended" }).where(eq(schema.cullinosTenants.id, tenantId));
  await db.update(schema.saasSubscriptions).set({ status: "suspended" }).where(eq(schema.saasSubscriptions.tenantId, tenantId));
